/**
 * The draft-09 adapter.
 *
 * Extension headers became a length-prefixed blob here and stayed one
 * through draft-16. Still unconditional: there are no type flags to make it
 * optional until draft-11.
 *
 * **This module is the boundary of the draft-09 chunk.** It is reached only
 * through `DRAFT_LOADERS[9]`'s `() => import('../drafts/draft09/index.js')`, and
 * the `@moqtap/codec/draft09` specifier below is a **static string literal** on
...
import { decodeDatagram, decodeMessage, redactAuthTokens } from '@moqtap/codec/draft09'
...
import { BLOCK_LENGTH, PRESENT_ALWAYS, type WalkDialect } from '../data-walk.js'

/**
 * draft-09's data-stream dialect, read off `drafts/draft09/data-streams.ts`.
 *
 ...
 */
export const draft: SupportedDraft = 9

/** The one adapter instance for draft-09. Stateless, so one is enough. */
export const DRAFT09_ADAPTER: DraftAdapter = /*#__PURE__*/ Object.freeze(
  makeAdapter(draft, PROTOCOL_STRINGS[9], RFC9000_READER, DIALECT, {
    decodeMessage,
    decodeDatagram,
    redactAuthTokens,
  }),
...
export { DRAFT09_ADAPTER as adapter }
