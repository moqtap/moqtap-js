 ...
 *
 * Bare `settle()` is a duration with a turn floor, not a turn count alone. The
 * tests that pin `metrics.intervalMs` assert on records that exist only once a
 ...
 * one and ~60 ms on the other. Turn-based, those passed on Windows and failed in
