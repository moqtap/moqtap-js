...
import type { SessionState, SessionPhase, TransitionResult } from '@moqtap/codec/session';

function createMockSession(initialPhase: SessionPhase = 'idle'): SessionState {
  let phase: SessionPhase = initialPhase;
  return {
    get phase() { return phase; },
    get role() { return 'client' as const; },
    get subscriptions() { return new Map(); },
    get announces() { return new Map(); },
    get legalOutgoing() { return new Set<MoqtMessageType>(); },
    get legalIncoming() { return new Set<MoqtMessageType>(); },
...
describe('TraceRecorder', () => {
  ...
  it('detail level includes stream events at headers level', () => {
    ...
    expect(trace.events.map(e => e.type)).toEqual([
      'stream-opened', 'object-header', 'stream-closed',
    ]);
  });
