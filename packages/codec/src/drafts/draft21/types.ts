// Draft-21 specific message types
...
export interface Draft21SetupOptions {
...
/**
 * FILL_PARAMETERS (0x23) value — NEW in draft-20 (Section 9.20.16).
 *
 ...
 */
export interface Draft21FillParameters {
...
export interface Draft21Params {
  ...
  subscriber_priority?: bigint // 0x20 uint8
  location_filter?: LocationFilter // 0x21 length-prefixed; see LocationFilter for the draft-21 shape
  group_order?: bigint // 0x22 uint8
  fill_parameters?: Draft21FillParameters // 0x23 length-prefixed nested (NEW in draft-20)
  // Section 3.3.2 permits each of the five more than once in a message: a
  // filter parameter carries one SetID, so two alternatives over the same
  // field need two parameters. A single filter is a one-element list.
  subgroup_filter?: readonly RangeFilter[] // 0x25 length-prefixed
...
export interface Draft21TrackProperties {
...
// Draft-21 message type tag union
export type Draft21MessageType =
  | 'setup'
  | 'subscribe'
  | 'subscribe_ok'
  | 'request_update'
...
// Base
export interface Draft21BaseMessage {
  readonly type: Draft21MessageType
}

// Setup — single unified message (0x2F00)
export interface Draft21Setup extends Draft21BaseMessage {
  readonly type: 'setup'
  readonly options: Draft21SetupOptions
}

export interface Draft21Subscribe extends Draft21BaseMessage {
  readonly type: 'subscribe'
  readonly request_id: bigint
  readonly track_namespace: string[]
  readonly track_name: string
  readonly parameters: Draft21Params
}

export interface Draft21SubscribeOk extends Draft21BaseMessage {
  readonly type: 'subscribe_ok'
  readonly track_alias: bigint
  readonly parameters: Draft21Params
  readonly track_properties: Draft21TrackProperties
}

export interface Draft21RequestUpdate extends Draft21BaseMessage {
 ...
 */
export interface Draft21PublishStateNotify extends Draft21BaseMessage {
  readonly type: 'publish_state_notify'
  readonly parameters: Draft21Params
}

export interface Draft21Publish extends Draft21BaseMessage {
...
export interface Draft21PublishDone extends Draft21BaseMessage {
...
export interface Draft21PublishNamespace extends Draft21BaseMessage {
...
export interface Draft21Namespace extends Draft21BaseMessage {
  readonly type: 'namespace'
  readonly namespace_suffix: string[]
}

export interface Draft21NamespaceDone extends Draft21BaseMessage {
  readonly type: 'namespace_done'
  readonly namespace_suffix: string[]
}

export interface Draft21SubscribeNamespace extends Draft21BaseMessage {
...
export interface Draft21SubscribeTracks extends Draft21BaseMessage {
...
export interface Draft21PublishSkipped extends Draft21BaseMessage {
 ...
 */
export interface Draft21Fetch extends Draft21BaseMessage {
  readonly type: 'fetch'
  readonly request_id: bigint
  readonly track_namespace: string[]
  readonly track_name: string
 ...
 * 1; or 0 to indicate the entire Group" is gone (draft-21 Section 9.12 with
 * Section 3.3.1). The same bytes mean a different object under the two drafts,
 * and nothing on the wire distinguishes them.
 */
export interface Draft21FetchOk extends Draft21BaseMessage {
...
export interface Draft21TrackStatus extends Draft21BaseMessage {
...
export interface Draft21RequestOk extends Draft21BaseMessage {
 ...
 *
 * draft-21 Section 9.4.1 names the namespace/name pair "the Redirect target"
...
export interface Draft21RequestError extends Draft21BaseMessage {
...
export interface Draft21GoAway extends Draft21BaseMessage {
  readonly type: 'goaway'
  readonly new_session_uri: string
  readonly timeout: bigint
}

// Union of all draft-21 control messages
export type Draft21Message =
  | Draft21Setup
  | Draft21Subscribe
  | Draft21SubscribeOk
...
export interface FetchStream {
  readonly type: 'fetch'
  /**
   * The Request ID of the message that opened the stream. In draft-21 that is
...
export type Draft21DataStream = SubgroupStream | DatagramObject | FetchStream
