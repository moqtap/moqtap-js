// Draft-21 error and status code registries.
//
...
// Section 13: treat it as INTERNAL_ERROR for its context, and MUST NOT close
// the session over it in a REQUEST_ERROR or PUBLISH_DONE. So none of the sets
// below is a decode gate -- see the note above them.

/** draft-21 Section 16.11.1, Table 18 (prose Section 6.6). */
...
/** draft-21 Section 16.11.2, Table 19 (prose Section 9.4.2). */
export const RequestErrorCode = {
  ...
  // joining-fetch mechanism (draft-19 Section 10.12.2). A draft-21 FETCH names
...
/** draft-21 Section 16.11.3, Table 20 (prose Section 9.9). */
export const PublishDoneCode = {
  InternalError: 0x0n,
  Unauthorized: 0x1n,
  TrackEnded: 0x2n,
  // 0x3 SUBSCRIPTION_ENDED — REMOVED in draft-20. The behaviour went with it:
  // Section 3.3.1 now says "A publisher does not end a subscription solely
...
/** draft-21 Section 16.11.4, Table 21 (prose Section 12.5) — unchanged from draft-19. */
 ...
 * Retired codepoints.
 *
 * draft-21 states no rule for what a receiver does with a code that is not in
 ...
 * on a connection that negotiated moqt-21. But Section 13's "MUST NOT close the
