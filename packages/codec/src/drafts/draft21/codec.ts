...
/** The largest value MOQT's vi64 can carry (draft-21 Section 8.1, nine-byte form). */
const MAX_U64 = 0xffffffffffffffffn

/**
 * `PUBLISH_DONE.Stream Count` "unknown" sentinel — draft-21 Section 9.9.
 *
...
function encodeSetupOptions(opts: Draft21SetupOptions, writer: BufferWriter): void {
...
function decodeSetupOptions(reader: BufferReader, payloadEnd: number): Draft21SetupOptions {
  const result: Draft21SetupOptions = {}
...
function encodeAuthorizationToken(token: AuthorizationToken, w: BufferWriter): void {
  ...
  if (aliasType === 1 || aliasType === 3) {
    ...
    // of the outer length-prefixed buffer (draft-21 Section 9.20.3).
 ...
 * tail of the Token structure — draft-21 Section 9.20.3, Figure 3: it is NOT
...
// ─── Message Parameter type codes (draft-21 Section 16.7, Table 13) ─────────
 ...
 * *separate parameter scope*. draft-21 Section 9.20.16 says "Parameters inside
...
// ─── Range Filter Encoding/Decoding (draft-21 Section 3.3.2) ────────────────
...
// ─── LOCATION_FILTER (0x21) — draft-21 Section 3.3.1 ───────────────────────
 ...
 * whole group" special case. draft-21 Section 3.3.1 says "A Location filter
 * specifies an inclusive range of Locations", and Section 9.11 repeats it for
 ...
 * from parsing, never from the byte length. draft-21 Section 3.3.1 says
 ...
 * not be minimally encoded (Section 8.1), so a Length of 2 is equally
...
function decodeLocationFilter(reader: BufferReader): LocationFilter {
  ...
  if (filter.start_group !== undefined && filter.end_group_delta !== undefined) {
    // Section 3.3.1: "If StartGroup + EndGroupDelta exceeds 2^64 - 1, the
 ...
 *
 * Read out of draft-21's per-parameter sections, not carried over from
 ...
 * inside FILL_PARAMETERS (Section 9.20.16). TRACK_PROPERTY_FILTER (0x29) is
...
function encodeParams(params: Draft21Params, writer: BufferWriter): void {
  ...
  if (params.fill_parameters !== undefined) {
    entries.push({
      ...
      encode: (w) => {
        ...
        // Section 9.20.16 calls it "a sequence of Parameters ... encoded as if
        ...
        const tmpW = new BufferWriter(64)
        encodeParams(params.fill_parameters as Draft21Params, tmpW)
        const raw = tmpW.finish()
        w.writeVarInt(BigInt(raw.byteLength))
        w.writeBytes(raw)
      },
  ...
  if (params.include_properties !== undefined) {
    entries.push({
      ...
      // uint8: one raw byte, 0 or 1 (draft-21 Section 9.20.22).
...
function decodeParams(reader: BufferReader, messageType: string): Draft21Params {
  const count = Number(reader.readVarInt())
  const result: Draft21Params = {}
  let prevType = 0n
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (paramType === PARAM_OBJECT_DELIVERY_TIMEOUT) {
      ...
      // Section 9.20.22: "The allowed values are 0 (do not send Properties) or
...
/**
 * Decode a FILL_PARAMETERS (0x23) value — draft-21 Section 9.20.16.
 *
 ...
 */
function decodeFillParameters(reader: BufferReader): Draft21FillParameters {
  const length = Number(reader.readVarInt())
  const valueBytes = reader.readBytes(length)
  ...
  return params as Draft21FillParameters
}
...
function encodeTrackProperties(props: Draft21TrackProperties, writer: BufferWriter): void {
  const entries: Array<{ type: bigint; encode: (w: BufferWriter) => void }> = []
...
function decodeTrackProperties(reader: BufferReader, payloadEnd: number): Draft21TrackProperties {
  const result: Draft21TrackProperties = {}
  const unknown: UnknownParam[] = []
  let prevType = 0n
...
function decodeRedirect(r: BufferReader): Redirect {
  ...
  // draft-21 Section 9.4.1 names this pair "the Redirect target". draft-19's
...
function encodeSetupPayload(msg: Draft21Message & { type: 'setup' }, w: BufferWriter): void {
  encodeSetupOptions(msg.options, w)
}

function encodeSubscribePayload(
  msg: Draft21Message & { type: 'subscribe' },
  w: BufferWriter,
): void {
...
function encodeSubscribeOkPayload(
  msg: Draft21Message & { type: 'subscribe_ok' },
  w: BufferWriter,
): void {
...
function encodeRequestUpdatePayload(
  msg: Draft21Message & { type: 'request_update' },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id)
  encodeParams(msg.parameters, w)
...
function encodePublishStateNotifyPayload(
  msg: Draft21Message & { type: 'publish_state_notify' },
  w: BufferWriter,
): void {
...
function encodePublishPayload(msg: Draft21Message & { type: 'publish' }, w: BufferWriter): void {
  w.writeVarInt(msg.request_id)
  w.writeTuple(msg.track_namespace)
  w.writeString(msg.track_name)
  w.writeVarInt(msg.track_alias)
...
function encodePublishDonePayload(
  msg: Draft21Message & { type: 'publish_done' },
  w: BufferWriter,
): void {
...
function encodePublishNamespacePayload(
  msg: Draft21Message & { type: 'publish_namespace' },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id)
  w.writeTuple(msg.track_namespace)
...
function encodeNamespacePayload(
  msg: Draft21Message & { type: 'namespace' },
  w: BufferWriter,
): void {
...
function encodeNamespaceDonePayload(
  msg: Draft21Message & { type: 'namespace_done' },
  w: BufferWriter,
): void {
  w.writeTuple(msg.namespace_suffix)
}
...
function encodeSubscribeNamespacePayload(
  msg: Draft21Message & { type: 'subscribe_namespace' },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id)
...
function encodeSubscribeTracksPayload(
  msg: Draft21Message & { type: 'subscribe_tracks' },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id)
  w.writeTuple(msg.namespace_prefix)
...
function encodePublishSkippedPayload(
  msg: Draft21Message & { type: 'publish_skipped' },
  w: BufferWriter,
): void {
  w.writeTuple(msg.namespace_suffix)
  w.writeString(msg.track_name)
...
/**
 * FETCH (0x16) — draft-21 Section 9.11, Figure 15.
 *
 ...
 */
function encodeFetchPayload(msg: Draft21Message & { type: 'fetch' }, w: BufferWriter): void {
  w.writeVarInt(msg.request_id)
  w.writeTuple(msg.track_namespace)
  w.writeString(msg.track_name)
  encodeParams(msg.parameters, w)
...
function encodeFetchOkPayload(msg: Draft21Message & { type: 'fetch_ok' }, w: BufferWriter): void {
  w.writeUint8(msg.end_of_track)
  // End Location, written as given. draft-19 wrote "the last Object, plus 1"
  // here and used an Object of 0 to mean the whole group; draft-21 Section
...
function encodeTrackStatusPayload(
  msg: Draft21Message & { type: 'track_status' },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id)
  w.writeTuple(msg.track_namespace)
...
function encodeRequestOkPayload(
  msg: Draft21Message & { type: 'request_ok' },
  w: BufferWriter,
): void {
  encodeParams(msg.parameters, w)
  encodeTrackProperties(msg.track_properties, w)
...
function encodeRequestErrorPayload(
  msg: Draft21Message & { type: 'request_error' },
  w: BufferWriter,
): void {
...
function encodeGoAwayPayload(msg: Draft21Message & { type: 'goaway' }, w: BufferWriter): void {
...
function decodeSetupPayload(r: BufferReader, payloadEnd: number): Draft21Message {
  const options = decodeSetupOptions(r, payloadEnd)
  return { type: 'setup', options }
}

function decodeSubscribePayload(r: BufferReader): Draft21Message {
...
function decodeSubscribeOkPayload(r: BufferReader, payloadEnd: number): Draft21Message {
...
function decodeRequestUpdatePayload(r: BufferReader): Draft21Message {
  const request_id = r.readVarInt()
...
function decodePublishStateNotifyPayload(r: BufferReader): Draft21Message {
  const parameters = decodeParams(r, 'publish_state_notify')
  return { type: 'publish_state_notify', parameters }
}

function decodePublishPayload(r: BufferReader, payloadEnd: number): Draft21Message {
  const request_id = r.readVarInt()
  const track_namespace = r.readTuple()
  const track_name = r.readString()
  const track_alias = r.readVarInt()
...
function decodePublishDonePayload(r: BufferReader): Draft21Message {
...
function decodePublishNamespacePayload(r: BufferReader): Draft21Message {
  const request_id = r.readVarInt()
  const track_namespace = r.readTuple()
...
function decodeNamespacePayload(r: BufferReader): Draft21Message {
  const namespace_suffix = r.readTuple()
  return { type: 'namespace', namespace_suffix }
}

function decodeNamespaceDonePayload(r: BufferReader): Draft21Message {
  const namespace_suffix = r.readTuple()
  return { type: 'namespace_done', namespace_suffix }
}

function decodeSubscribeNamespacePayload(r: BufferReader): Draft21Message {
  const request_id = r.readVarInt()
...
function decodeSubscribeTracksPayload(r: BufferReader): Draft21Message {
  const request_id = r.readVarInt()
  const namespace_prefix = r.readTuple()
...
function decodePublishSkippedPayload(r: BufferReader): Draft21Message {
  const namespace_suffix = r.readTuple()
  const track_name = r.readString()
  return { type: 'publish_skipped', namespace_suffix, track_name }
}

function decodeFetchPayload(r: BufferReader): Draft21Message {
  ...
  // decoder would read as a Fetch Type. See the Draft21Fetch doc comment.
  const request_id = r.readVarInt()
  const track_namespace = r.readTuple()
  const track_name = r.readString()
...
function decodeFetchOkPayload(r: BufferReader, payloadEnd: number): Draft21Message {
...
function decodeTrackStatusPayload(r: BufferReader): Draft21Message {
  const request_id = r.readVarInt()
  const track_namespace = r.readTuple()
  const track_name = r.readString()
...
function decodeRequestOkPayload(r: BufferReader, payloadEnd: number): Draft21Message {
  const parameters = decodeParams(r, 'request_ok')
  const track_properties = decodeTrackProperties(r, payloadEnd)
  return { type: 'request_ok', parameters, track_properties }
}

function decodeRequestErrorPayload(r: BufferReader, payloadEnd: number): Draft21Message {
...
function decodeGoAwayPayload(r: BufferReader): Draft21Message {
...
/**
 * Encode a draft-21 control message: type (vi64) + length (uint16 BE) + payload.
 */
export function encodeMessage(message: Draft21Message): Uint8Array {
...
function encodePayload(msg: Draft21Message, w: BufferWriter): void {
  switch (msg.type) {
    case 'setup':
    ...
    default: {
      ...
      throw new Error(`Unhandled message type: ${(_exhaustive as Draft21Message).type}`)
    }
  }
}

/**
 * Decode a draft-21 control message from bytes (type + uint16 length + payload).
 */
export function decodeMessage(bytes: Uint8Array): DecodeResult<Draft21Message> {
  try {
...
export function createStreamDecoder(): TransformStream<Uint8Array, Draft21Message> {
  let buffer = new Uint8Array(0)
  let offset = 0

  return new TransformStream<Uint8Array, Draft21Message>({
...
export interface Draft21Codec extends BaseCodec<Draft21Message> {
  readonly draft: '21'
  ...
  decodeDataStream(
    ...
    bytes: Uint8Array,
  ): DecodeResult<Draft21DataStream>
  createStreamDecoder(): TransformStream<Uint8Array, Draft21Message>
...
export function createDraft21Codec(): Draft21Codec {
  return {
    draft: '21',
    encodeMessage,
    decodeMessage,
...
/**
 * Overwrite every Authorization Token value in a draft-21 control frame.
 *
