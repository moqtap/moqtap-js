 ...
 *
 * draft-21 keeps the draft-17 shape where a response is tied to its request by
 * the stream it arrives on rather than by a Request ID field. Several draft-21
 ...
 * the session with a PROTOCOL_VIOLATION" (Section 9.10) — so the FSM has to
 * know which stream a message arrived on to enforce them. That is what
 * {@link Draft21SessionFSM.receiveOn} and {@link Draft21SessionFSM.sendOn} are
 * for.
 */
...
/**
 * One fill fetch stream — draft-21 Section 3.4, new in draft-20.
 *
...
/** Draft-21 subscription bookkeeping beyond the draft-agnostic SubscriptionState. */
interface SubscriptionMeta {
  publisherSide: PublisherSide
  /**
   * Forward State, Section 3.1 / Section 9.20.19. Defaults to 1; FORWARD=0
   * turns delivery off. It gates fill fetch streams — see
   * {@link Draft21SessionFSM.applyFillParameters}.
   */
  forwardState: 0 | 1
  /**
   * Every fill fetch stream this subscription has, keyed by the Request ID
   * that opened it. Plural on purpose: Section 3.4 says "a subscription can
   ...
   * stream per subscription is wrong for draft-21.
   */
...
interface RequestMeta {
   ...
   * or REQUEST_ERROR (Section 9.1.7). PUBLISH_STATE_NOTIFY deliberately
...
function violation(
  code: ProtocolViolation<Draft21MessageType>['code'],
  message: string,
  currentPhase: SessionPhase,
  offendingMessage: Draft21MessageType,
): ProtocolViolation<Draft21MessageType> {
  return { code, message, currentPhase, offendingMessage }
}

function hasFillParameters(params: Draft21Params | undefined): Draft21FillParameters | undefined {
  return params?.fill_parameters
}

export class Draft21SessionFSM {
  ...
  get legalOutgoing(): ReadonlySet<Draft21MessageType> {
    return getLegalOutgoing(this._phase, this._role)
  }

  get legalIncoming(): ReadonlySet<Draft21MessageType> {
  ...
  private checkDuplicateRequestId(
    requestId: bigint,
    msgType: Draft21MessageType,
  ): ProtocolViolation<Draft21MessageType> | null {
  ...
  validateOutgoing(message: Draft21Message): ValidationResult<Draft21MessageType> {
  ...
  receive(message: Draft21Message): TransitionResult<Draft21MessageType> {
    return this.applyTransition(message, 'inbound', undefined)
  }

  send(message: Draft21Message): TransitionResult<Draft21MessageType> {
   ...
   * direction rules of Section 9.10 cannot be checked at all, and the
   * response-matching below falls back to FIFO guessing.
   */
  receiveOn(requestId: bigint, message: Draft21Message): TransitionResult<Draft21MessageType> {
    return this.applyTransition(message, 'inbound', requestId)
  }

  /** Send a message on a known request stream. See {@link receiveOn}. */
  sendOn(requestId: bigint, message: Draft21Message): TransitionResult<Draft21MessageType> {
    return this.applyTransition(message, 'outbound', requestId)
  }

  private applyTransition(
    message: Draft21Message,
    direction: 'inbound' | 'outbound',
    streamRequestId: bigint | undefined,
  ): TransitionResult<Draft21MessageType> {
  ...
  private handleSetup(
    ...
    const setup = message as import('./types.js').Draft21Setup
    // Section 9.1.7: absent, MAX_REQUEST_UPDATES defaults to 0, which means
  ...
  private handleGoAway(
    ...
    const goaway = message as import('./types.js').Draft21GoAway
    sideEffects.push({
  ...
  private requireReady(msgType: Draft21MessageType): ProtocolViolation<Draft21MessageType> | null {
    if (this._phase !== 'ready' && this._phase !== 'draining') {
      return violation(
        this._phase === 'idle' || this._phase === 'setup'
          ? 'MESSAGE_BEFORE_SETUP'
  ...
  private handleSubscribe(
    ...
    const sub = message as import('./types.js').Draft21Subscribe
    ...
    // FORWARD defaults to 1 (Section 9.20.19).
  ...
  /**
   * Open a fill fetch stream if this message asked for one — draft-21 Sections
  ...
  private handleRequestUpdate(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const update = message as import('./types.js').Draft21RequestUpdate
    ...
    // request being updated (Section 6.4.2.1), so a caller that does not pass the
    ...
    if (target !== undefined) {
      // Section 9.1.7: "If an endpoint receives a REQUEST_UPDATE on a stream
    ...
    if (targetId !== undefined && target !== undefined) {
      if (target.kind !== 'subscription') {
        // Section 9.20.16: FILL_PARAMETERS "MAY appear in a SUBSCRIBE or
        ...
        // it updates — so the scope rule of Section 9.20.1 lands here, where
        ...
        if (meta !== undefined) {
          // Sticky parameters (Section 9.5): an omitted parameter keeps its
          ...
          // Section 3.4.1: "A REQUEST_UPDATE that does not carry
  ...
  /**
   * PUBLISH_STATE_NOTIFY — draft-21 Section 9.10, new in draft-20.
   *
  ...
  private handlePublish(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const pub = message as import('./types.js').Draft21Publish
    ...
    // A PUBLISH initiates a subscription (Section 9.8), so it is a
    // subscription request stream for the purposes of Section 9.10, and it
  ...
  private handlePublishDone(
    ...
    // Section 3.4.1: "When the subscription is cancelled, the publisher MUST
    // reset any open fill fetch streams." They also count toward
    // PUBLISH_DONE.Stream Count (Section 9.9), so a receiver reconciling that
  ...
  private handleFetch(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const fetch = message as import('./types.js').Draft21Fetch
    const dupErr = this.checkDuplicateRequestId(fetch.request_id, message.type)
    if (dupErr) return { ok: false, violation: dupErr }
    this._requestIds.add(fetch.request_id)
    // draft-21 has no Joining FETCH: a FETCH names its own track inline and
    // never references another request (Section 9.11). Nothing to resolve
  ...
  private handleRequestOk(
    ...
    if (streamRequestId !== undefined) {
      ...
      const pub = this._publishes.get(streamRequestId)
      if (pub !== undefined && pub.phase === 'pending') {
        // PUBLISH_OK. In draft-21 it can no longer carry subscription
  ...
  private handleGenericRequest(
    ...
    this._requests.set(requestId, {
      kind,
      // Only subscriptions have a publisher in the Section 9.10 sense; the
  ...
  private handleReadyPhaseMessage(message: Draft21Message): TransitionResult<Draft21MessageType> {
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    return { ok: true, phase: this._phase, sideEffects: [] }
  }
