...
// ─── Fetch stream Serialization Flags (draft-21 Section 11.4.1, Table 7) ────
...
/**
 * End of Timed-Out Range — NEW in draft-20 (Sections 11.4.1, 11.4.4.2).
 *
...
// ─── SUBGROUP_HEADER Type Flags (draft-21 Section 11.3.1) ───────────────────
//
// | Bit | Mask | Meaning
...
// |  6  | 0x40 | FIRST_OBJECT
//
// These are draft-21's OWN rules, computed from Section 11.3.1's three
// conditions. They are NOT the datagram rules (Section 11.2.1): bit 4 is
...
// SPEC-DELTA Section 9 item 3 flags exactly this conflation as a mistake, so
...
// ─── OBJECT_DATAGRAM Type Flags (draft-21 Section 11.2.1) ───────────────────
//
// | Bit | Mask | Meaning
// |  0  | 0x01 | PROPERTIES
// |  1  | 0x02 | END_OF_GROUP
...
//
// Section 11.2.1's invalid values: STATUS and END_OF_GROUP both set; bit 4 set;
...
/** The largest value a Group ID or Object ID can take (draft-21 Section 8.1). */
const MAX_U64 = 0xffffffffffffffffn

export function encodeSubgroupStream(stream: SubgroupStream): Uint8Array {
  const w = new BufferWriter()
  const streamType = stream.headerType
  // DECISION (DECISIONS.md D5): strict on send. Section 8.1 permits
  // non-minimal encodings, and Section 11.3.1 words its third invalidity rule
  ...
  for (const obj of stream.objects) {
    // Section 11.3.1: "Object ID = previous Object ID + Object ID Delta + 1",
...
export function encodeDatagram(dg: DatagramObject): Uint8Array {
  ...
  if (propertiesPresent) {
    if (dg.objectProperties && Object.keys(dg.objectProperties).length > 0) {
      ...
      // Section 11.2.1 makes this a PROTOCOL_VIOLATION on receipt. An object
...
export function encodeFetchStream(stream: FetchStream): Uint8Array {
  ...
  for (const obj of stream.objects) {
    ...
    if (flags >= 0x80) {
      ...
      // Section 11.4.1.1 delta arithmetic applies to the marker's two fields.
      // All three marker values carry the low bits 0x0C — the ordinary
      // "Group ID Delta present, Object ID Delta present" pattern — and
      // Section 11.4.1.2 says only that "the Group ID and Object ID fields are
      ...
      // encoded as 0. Section 11.4.1.2 lists what is absent — "Subgroup ID,
      ...
      if (flags & 0x08) {
        // Ascending order: Group ID = prior + delta + 1. Section 11.4.1.1
...
/**
 * The value to write into the Object ID Delta slot, per draft-21 Section
...
/**
 * Resolve a Group ID Delta, per draft-21 Section 11.4.1.1.
 *
 ...
 *
 * `decodeSubgroupStream` and `createSubgroupStreamDecoder` share this single
 * reader so they cannot disagree about a header: the Type Flags, the
 * end-of-group and first-object flags, and the Subgroup ID under the mode that
 * derives it from the first Object all come from here. Two readers drift.
 */
interface SubgroupHeaderFields {
  readonly streamType: number
  readonly propertiesPresent: boolean
  /** Subgroup Mode 0b01: the Subgroup ID is the first Object's ID. */
...
function readSubgroupHeader(r: BufferReader): SubgroupHeaderFields {
  ...
  // vi64 and Section 8.1 allows non-minimal encodings, so 0x10 may legally
  ...
  if (flagsError !== null) {
    throw new DecodeError('CONSTRAINT_VIOLATION', flagsError, 0)
  }
 ...
 * buffer and passes that window's position, which is what makes `byteOffset`
 * mean the same thing on both paths instead of being hardcoded to zero on one
 * of them.
 *
 * Throws `UNEXPECTED_END` when the Object is not yet complete, which the
 * incremental decoder treats as "wait for more bytes" rather than as an error.
 */
function readSubgroupObject(
  ...
  const delta = r.readVarInt()
  // Section 11.3.1: "Object ID = previous Object ID + Object ID Delta + 1", or
  // the delta itself for the first Object on the stream.
  ...
  if (payloadLength === 0) {
    // Section 11.3.1: "The Object Status field is only sent if the Object
    // Payload Length is zero." Skipping it leaves the varint to be read as the
    // next Object's Object ID Delta, which desynchronises the rest of the
    // stream silently.
    status = r.readVarInt()
...
export function decodeDatagram(bytes: Uint8Array): DecodeResult<DatagramObject> {
  try {
    ...
    if (propertiesPresent) {
      const propsLen = Number(r.readVarInt())
      if (propsLen === 0) {
        // Section 11.2.1: "If an endpoint receives a datagram with the
    ...
    if (isStatus) {
      ...
      if (propertiesPresent && objectStatus !== OBJECT_STATUS_NORMAL) {
        // Section 11.2.1: STATUS and PROPERTIES both set with a status other
 ...
 * cannot report a different shape for the same bytes: the Serialization Flags,
 * the Group and Subgroup ID, the Priority, the resolved deltas and a true
 * `byteOffset` all come from this one reader, and DATAGRAM-mode Object IDs
 * resolve against the state it carries.
 *
...
function readFetchObject(r: BufferReader, base: number, st: FetchObjectState): FetchObjectPayload {
  ...
  // Section 11.4.1.1: absent an Object ID Delta, Object ID is the prior
  ...
  if (flags >= 0x80) {
    // End-of-Range marker. Section 11.4.1: "Any other value is a
    // PROTOCOL_VIOLATION."
    ...
    // Section 11.4.1.2: prior Group ID and prior Object ID come FROM the
  ...
  if (flags & 0x40) {
    ...
    if (groupIdPresent) {
      ...
      // groupId = st.prevGroupId + delta + 1 (ascending).
    ...
    if (objectIdPresent) {
      ...
      objectId = resolveObjectId(groupIdPresent, objectDelta, st.prevObjectId)
    }
    if (objectId > MAX_U64) {
      throw new DecodeError('CONSTRAINT_VIOLATION', 'computed Object ID exceeds 2^64 - 1', r.offset)
    }
    if (priorityPresent) {
  ...
  if (subgroupEncoding === 0x00) {
    ...
    subgroupId = subgroupEncoding === 0x01 ? st.prevSubgroupId : st.prevSubgroupId + 1n
  } else {
    subgroupId = r.readVarInt()
  }
  ...
  // subscription-delivered objects (Section 11.1.2). That is also the
...
export function decodeFetchStream(bytes: Uint8Array): DecodeResult<FetchStream> {
  try {
    ...
    // Section 11.4.1: "all objects on the stream belong to the track requested
    // in the message identified by Request ID" — "the message", not "the FETCH
    // message", because a fill fetch stream (Section 3.4) names a SUBSCRIBE
 ...
 * room rather than a fresh `unread + chunk.length` array and two copies.
 *
...
class StreamBuffer {
  ...
  append(chunk: Uint8Array): void {
    if (this.len + chunk.length > this.buf.length) {
      ...
      // half free. Sizing to `needed` would leave no room at all, so the next
      // chunk would reallocate too and the buffer would reallocate on every
      // single chunk, which is the cost it exists to avoid. Capacity never
      // shrinks, so it settles at roughly twice the largest `live + chunk`
...
> {
  ...
  let headerEmitted = false
  /**
   * The delta state the Objects carry between them. DATAGRAM-mode Object IDs
   * resolve against the previous Object's ID, so a decoder that keeps no state
   * here reports them wrong.
   */
  const st = newFetchObjectState()
...
export function createDataStreamDecoder(): TransformStream<Uint8Array, DataStreamEvent> {
   ...
   * bytes arrive. Buffering chunks locally and decoding the whole stream in
   * `flush` instead would deliver every event at end-of-stream, which on a
