...
function decodeSetupParams(r: BufferReader): Draft11SetupParams {
  const count = Number(r.readVarInt());
  const result: Draft11SetupParams = {};
...
function encodeParams(params: Draft11Params, w: BufferWriter): void {
  ...
  if (params.delivery_timeout !== undefined) {
    // DELIVERY_TIMEOUT = 0x02 (even → bare varint)
...
function decodeParams(r: BufferReader): Draft11Params {
  const count = Number(r.readVarInt());
  const result: Draft11Params = {};
  const unknown: UnknownParam[] = [];
...
function encodeClientSetupPayload(
  msg: Draft11Message & { type: "client_setup" },
  w: BufferWriter,
): void {
...
function encodeServerSetupPayload(
  msg: Draft11Message & { type: "server_setup" },
  w: BufferWriter,
): void {
...
function encodeSubscribePayload(
  msg: Draft11Message & { type: "subscribe" },
  w: BufferWriter,
): void {
...
function encodeSubscribeOkPayload(
  msg: Draft11Message & { type: "subscribe_ok" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
...
function encodeSubscribeErrorPayload(
  msg: Draft11Message & { type: "subscribe_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
...
function encodeSubscribeUpdatePayload(
  msg: Draft11Message & { type: "subscribe_update" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
  w.writeVarInt(msg.start_group);
...
function encodeSubscribeDonePayload(
  msg: Draft11Message & { type: "subscribe_done" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
...
function encodeUnsubscribePayload(
  msg: Draft11Message & { type: "unsubscribe" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
}

function encodeAnnouncePayload(msg: Draft11Message & { type: "announce" }, w: BufferWriter): void {
  w.writeVarInt(msg.request_id);
  w.writeTuple(msg.track_namespace);
  encodeParams(msg.parameters, w);
}
...
function encodeAnnounceOkPayload(
  msg: Draft11Message & { type: "announce_ok" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
}
...
function encodeAnnounceErrorPayload(
  msg: Draft11Message & { type: "announce_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
  w.writeVarInt(msg.error_code);
...
function encodeUnannouncePayload(
  msg: Draft11Message & { type: "unannounce" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
}
...
function encodeAnnounceCancelPayload(
  msg: Draft11Message & { type: "announce_cancel" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeVarInt(msg.error_code);
...
function encodeSubscribeAnnouncesPayload(
  msg: Draft11Message & { type: "subscribe_announces" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
...
function encodeSubscribeAnnouncesOkPayload(
  msg: Draft11Message & { type: "subscribe_announces_ok" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
}
...
function encodeSubscribeAnnouncesErrorPayload(
  msg: Draft11Message & { type: "subscribe_announces_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
  w.writeVarInt(msg.error_code);
...
function encodeUnsubscribeAnnouncesPayload(
  msg: Draft11Message & { type: "unsubscribe_announces" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace_prefix);
}

function encodeFetchPayload(msg: Draft11Message & { type: "fetch" }, w: BufferWriter): void {
  w.writeVarInt(msg.request_id);
  w.writeUint8(msg.subscriber_priority);
  w.writeVarInt(msg.group_order);
...
function encodeFetchOkPayload(msg: Draft11Message & { type: "fetch_ok" }, w: BufferWriter): void {
  w.writeVarInt(msg.request_id);
  w.writeVarInt(msg.group_order);
...
function encodeFetchErrorPayload(
  msg: Draft11Message & { type: "fetch_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
  w.writeVarInt(msg.error_code);
...
function encodeFetchCancelPayload(
  msg: Draft11Message & { type: "fetch_cancel" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
}
...
function encodeTrackStatusRequestPayload(
  msg: Draft11Message & { type: "track_status_request" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
  w.writeTuple(msg.track_namespace);
...
function encodeTrackStatusPayload(
  msg: Draft11Message & { type: "track_status" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
  w.writeVarInt(msg.status_code);
...
function encodeGoAwayPayload(msg: Draft11Message & { type: "goaway" }, w: BufferWriter): void {
  w.writeString(msg.new_session_uri);
}

function encodeMaxRequestIdPayload(
  msg: Draft11Message & { type: "max_request_id" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.request_id);
}
...
function encodeRequestsBlockedPayload(
  msg: Draft11Message & { type: "requests_blocked" },
  w: BufferWriter,
): void {
...
function decodeClientSetupPayload(r: BufferReader): Draft11Message {
  const numVersions = Number(r.readVarInt());
  if (numVersions === 0) {
    throw new DecodeError("INVALID_VALUE", "supported_versions must not be empty", r.offset);
  }
...
function decodeServerSetupPayload(r: BufferReader): Draft11Message {
  const selected_version = r.readVarInt();
  const parameters = decodeSetupParams(r);
  return { type: "server_setup", selected_version, parameters };
}

function decodeSubscribePayload(r: BufferReader): Draft11Message {
  ...
  return msg as unknown as Draft11Message;
}

function decodeSubscribeOkPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
...
function decodeSubscribeErrorPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
  const track_alias = r.readVarInt();
  return { type: "subscribe_error", request_id, error_code, reason_phrase, track_alias };
}

function decodeSubscribeUpdatePayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
...
function decodeSubscribeDonePayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
...
function decodeUnsubscribePayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  return { type: "unsubscribe", request_id };
}

function decodeAnnouncePayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const track_namespace = r.readTuple();
  const parameters = decodeParams(r);
...
function decodeAnnounceOkPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  return { type: "announce_ok", request_id };
}

function decodeAnnounceErrorPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeUnannouncePayload(r: BufferReader): Draft11Message {
  const track_namespace = r.readTuple();
  return { type: "unannounce", track_namespace };
}

function decodeAnnounceCancelPayload(r: BufferReader): Draft11Message {
  const track_namespace = r.readTuple();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeSubscribeAnnouncesPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
...
function decodeSubscribeAnnouncesOkPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  return { type: "subscribe_announces_ok", request_id };
}

function decodeSubscribeAnnouncesErrorPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeUnsubscribeAnnouncesPayload(r: BufferReader): Draft11Message {
  const track_namespace_prefix = r.readTuple();
  return { type: "unsubscribe_announces", track_namespace_prefix };
}

function decodeFetchPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const subscriber_priority = r.readUint8();
  const group_order = r.readVarInt();
...
function decodeFetchOkPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const group_order = r.readVarInt();
...
function decodeFetchErrorPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeFetchCancelPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  return { type: "fetch_cancel", request_id };
}

function decodeTrackStatusRequestPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const track_namespace = r.readTuple();
  const track_name = r.readString();
  const parameters = decodeParams(r);
...
function decodeTrackStatusPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  const status_code = r.readVarInt();
  const group = r.readVarInt();
  const object = r.readVarInt();
...
function decodeGoAwayPayload(r: BufferReader): Draft11Message {
  const new_session_uri = r.readString();
  return { type: "goaway", new_session_uri };
}

function decodeMaxRequestIdPayload(r: BufferReader): Draft11Message {
  const request_id = r.readVarInt();
  return { type: "max_request_id", request_id };
}

function decodeRequestsBlockedPayload(r: BufferReader): Draft11Message {
...
const payloadDecoders: ReadonlyMap<bigint, (r: BufferReader) => Draft11Message> = new Map([
 ...
 */
export function encodeMessage(message: Draft11Message): Uint8Array {
...
function encodePayload(msg: Draft11Message, w: BufferWriter): void {
  switch (msg.type) {
    case "client_setup":
    ...
    default: {
      ...
      throw new Error(`Unhandled message type: ${(_exhaustive as Draft11Message).type}`);
    }
  }
}
...
export interface Draft11Codec extends BaseCodec<Draft11Message> {
...
export function createDraft11Codec(): Draft11Codec {
  return {
    draft: "draft-ietf-moq-transport-11",
    encodeMessage,
    decodeMessage,
