...
function encodeParams(params: Draft11Params, w: BufferWriter): void {
  ...
  if (params.max_cache_duration !== undefined) {
    ...
    w.writeVarInt(params.max_cache_duration)
  }
  if (params.unknown) {
    for (const u of params.unknown) {
      const id = BigInt(u.id)
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (paramType % 2n === 0n) {
      ...
      if (paramType === PARAM_AUTHORIZATION_TOKEN) {
        ...
        noteAuthTokenValue(startOff, length, tokenBytes)
...
/**
 * Overwrite every Authorization Token value in a draft-11 control frame.
 *
