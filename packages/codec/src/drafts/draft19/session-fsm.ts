...
import type { Draft19Message, Draft19MessageType } from './types.js'

function violation(
  code: ProtocolViolation<Draft19MessageType>['code'],
  message: string,
  currentPhase: SessionPhase,
  offendingMessage: Draft19MessageType,
): ProtocolViolation<Draft19MessageType> {
  return { code, message, currentPhase, offendingMessage }
}

export class Draft19SessionFSM {
  ...
  get legalOutgoing(): ReadonlySet<Draft19MessageType> {
    return getLegalOutgoing(this._phase, this._role)
  }

  get legalIncoming(): ReadonlySet<Draft19MessageType> {
    return getLegalIncoming(this._phase, this._role)
  }

  private checkDuplicateRequestId(
    requestId: bigint,
    msgType: Draft19MessageType,
  ): ProtocolViolation<Draft19MessageType> | null {
  ...
  validateOutgoing(message: Draft19Message): ValidationResult<Draft19MessageType> {
  ...
  receive(message: Draft19Message): TransitionResult<Draft19MessageType> {
    return this.applyTransition(message, 'inbound')
  }

  send(message: Draft19Message): TransitionResult<Draft19MessageType> {
    return this.applyTransition(message, 'outbound')
  }

  private applyTransition(
    message: Draft19Message,
    direction: 'inbound' | 'outbound',
  ): TransitionResult<Draft19MessageType> {
  ...
  private handleSetup(direction: 'inbound' | 'outbound'): TransitionResult<Draft19MessageType> {
    if (this._phase === 'idle') {
  ...
  private handleGoAway(
    ...
    const goaway = message as import('./types.js').Draft19GoAway
    sideEffects.push({
  ...
  private requireReady(msgType: Draft19MessageType): ProtocolViolation<Draft19MessageType> | null {
    if (this._phase !== 'ready' && this._phase !== 'draining') {
      return violation(
        this._phase === 'idle' || this._phase === 'setup'
          ? 'MESSAGE_BEFORE_SETUP'
  ...
  private handleSubscribe(
    ...
    const sub = message as import('./types.js').Draft19Subscribe
  ...
  private handleRequestUpdate(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const update = message as import('./types.js').Draft19RequestUpdate
  ...
  private handlePublish(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const pub = message as import('./types.js').Draft19Publish
  ...
  private handleFetch(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const fetch = message as import('./types.js').Draft19Fetch
  ...
  private handleFetchOk(sideEffects: SideEffect[]): TransitionResult<Draft19MessageType> {
  ...
  private handleRequestError(sideEffects: SideEffect[]): TransitionResult<Draft19MessageType> {
  ...
  private handleRequestOk(sideEffects: SideEffect[]): TransitionResult<Draft19MessageType> {
  ...
  private handleReadyPhaseMessage(message: Draft19Message): TransitionResult<Draft19MessageType> {
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    return { ok: true, phase: this._phase, sideEffects: [] }
  }
