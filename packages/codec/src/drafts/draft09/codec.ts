...
function decodeSetupParams(r: BufferReader): Draft09SetupParams {
  const count = Number(r.readVarInt());
  const result: Draft09SetupParams = {};
...
function encodeParams(params: Draft09Params, w: BufferWriter): void {
...
function decodeParams(r: BufferReader): Draft09Params {
  const count = Number(r.readVarInt());
  const result: Draft09Params = {};
  const unknown: UnknownParam[] = [];
...
function encodeClientSetupPayload(
  msg: Draft09Message & { type: "client_setup" },
  w: BufferWriter,
): void {
...
function encodeServerSetupPayload(
  msg: Draft09Message & { type: "server_setup" },
  w: BufferWriter,
): void {
...
function encodeSubscribePayload(
  msg: Draft09Message & { type: "subscribe" },
  w: BufferWriter,
): void {
...
function encodeSubscribeOkPayload(
  msg: Draft09Message & { type: "subscribe_ok" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
...
function encodeSubscribeErrorPayload(
  msg: Draft09Message & { type: "subscribe_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
...
function encodeSubscribeUpdatePayload(
  msg: Draft09Message & { type: "subscribe_update" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeVarInt(msg.start_group);
...
function encodeSubscribeDonePayload(
  msg: Draft09Message & { type: "subscribe_done" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
...
function encodeUnsubscribePayload(
  msg: Draft09Message & { type: "unsubscribe" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
}

function encodeAnnouncePayload(msg: Draft09Message & { type: "announce" }, w: BufferWriter): void {
  w.writeTuple(msg.track_namespace);
  encodeParams(msg.parameters, w);
}

function encodeAnnounceOkPayload(
  msg: Draft09Message & { type: "announce_ok" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
}
...
function encodeAnnounceErrorPayload(
  msg: Draft09Message & { type: "announce_error" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeVarInt(msg.error_code);
...
function encodeUnannouncePayload(
  msg: Draft09Message & { type: "unannounce" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
}
...
function encodeAnnounceCancelPayload(
  msg: Draft09Message & { type: "announce_cancel" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeVarInt(msg.error_code);
...
function encodeSubscribeAnnouncesPayload(
  msg: Draft09Message & { type: "subscribe_announces" },
  w: BufferWriter,
): void {
...
function encodeSubscribeAnnouncesOkPayload(
  msg: Draft09Message & { type: "subscribe_announces_ok" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace_prefix);
}
...
function encodeSubscribeAnnouncesErrorPayload(
  msg: Draft09Message & { type: "subscribe_announces_error" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace_prefix);
  w.writeVarInt(msg.error_code);
...
function encodeUnsubscribeAnnouncesPayload(
  msg: Draft09Message & { type: "unsubscribe_announces" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace_prefix);
}

function encodeFetchPayload(msg: Draft09Message & { type: "fetch" }, w: BufferWriter): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeUint8(msg.subscriber_priority);
  w.writeUint8(msg.group_order);
...
function encodeFetchOkPayload(msg: Draft09Message & { type: "fetch_ok" }, w: BufferWriter): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeUint8(msg.group_order);
...
function encodeFetchErrorPayload(
  msg: Draft09Message & { type: "fetch_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeVarInt(msg.error_code);
...
function encodeFetchCancelPayload(
  msg: Draft09Message & { type: "fetch_cancel" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
}
...
function encodeTrackStatusRequestPayload(
  msg: Draft09Message & { type: "track_status_request" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeString(msg.track_name);
...
function encodeTrackStatusPayload(
  msg: Draft09Message & { type: "track_status" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeString(msg.track_name);
...
function encodeGoAwayPayload(msg: Draft09Message & { type: "goaway" }, w: BufferWriter): void {
  w.writeString(msg.new_session_uri);
}

function encodeMaxSubscribeIdPayload(
  msg: Draft09Message & { type: "max_subscribe_id" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
}
...
function encodeSubscribesBlockedPayload(
  msg: Draft09Message & { type: "subscribes_blocked" },
  w: BufferWriter,
): void {
...
function decodeClientSetupPayload(r: BufferReader): Draft09Message {
...
function decodeServerSetupPayload(r: BufferReader): Draft09Message {
  const selected_version = r.readVarInt();
  const parameters = decodeSetupParams(r);
  return { type: "server_setup", selected_version, parameters };
}

function decodeSubscribePayload(r: BufferReader): Draft09Message {
  ...
  return msg as unknown as Draft09Message;
}

function decodeSubscribeOkPayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
...
function decodeSubscribeErrorPayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
...
function decodeSubscribeUpdatePayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
...
function decodeSubscribeDonePayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
...
function decodeUnsubscribePayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
  return { type: "unsubscribe", subscribe_id };
}

function decodeAnnouncePayload(r: BufferReader): Draft09Message {
  const track_namespace = r.readTuple();
  const parameters = decodeParams(r);
  return { type: "announce", track_namespace, parameters };
}

function decodeAnnounceOkPayload(r: BufferReader): Draft09Message {
  const track_namespace = r.readTuple();
  return { type: "announce_ok", track_namespace };
}

function decodeAnnounceErrorPayload(r: BufferReader): Draft09Message {
  const track_namespace = r.readTuple();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeUnannouncePayload(r: BufferReader): Draft09Message {
  const track_namespace = r.readTuple();
  return { type: "unannounce", track_namespace };
}

function decodeAnnounceCancelPayload(r: BufferReader): Draft09Message {
  const track_namespace = r.readTuple();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeSubscribeAnnouncesPayload(r: BufferReader): Draft09Message {
  const track_namespace_prefix = r.readTuple();
  const parameters = decodeParams(r);
  return { type: "subscribe_announces", track_namespace_prefix, parameters };
}

function decodeSubscribeAnnouncesOkPayload(r: BufferReader): Draft09Message {
  const track_namespace_prefix = r.readTuple();
  return { type: "subscribe_announces_ok", track_namespace_prefix };
}

function decodeSubscribeAnnouncesErrorPayload(r: BufferReader): Draft09Message {
  const track_namespace_prefix = r.readTuple();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeUnsubscribeAnnouncesPayload(r: BufferReader): Draft09Message {
  const track_namespace_prefix = r.readTuple();
  return { type: "unsubscribe_announces", track_namespace_prefix };
}

function decodeFetchPayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
  const subscriber_priority = r.readUint8();
  const group_order = r.readUint8();
...
function decodeFetchOkPayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
  const group_order = r.readUint8();
...
function decodeFetchErrorPayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeFetchCancelPayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
  return { type: "fetch_cancel", subscribe_id };
}

function decodeTrackStatusRequestPayload(r: BufferReader): Draft09Message {
  const track_namespace = r.readTuple();
  const track_name = r.readString();
  return { type: "track_status_request", track_namespace, track_name };
}

function decodeTrackStatusPayload(r: BufferReader): Draft09Message {
  const track_namespace = r.readTuple();
  const track_name = r.readString();
  const status_code = r.readVarInt();
...
function decodeGoAwayPayload(r: BufferReader): Draft09Message {
  const new_session_uri = r.readString();
  return { type: "goaway", new_session_uri };
}

function decodeMaxSubscribeIdPayload(r: BufferReader): Draft09Message {
  const subscribe_id = r.readVarInt();
  return { type: "max_subscribe_id", subscribe_id };
}

function decodeSubscribesBlockedPayload(r: BufferReader): Draft09Message {
...
const payloadDecoders: ReadonlyMap<bigint, (r: BufferReader) => Draft09Message> = new Map([
...
export function encodeMessage(message: Draft09Message): Uint8Array {
...
function encodePayload(msg: Draft09Message, w: BufferWriter): void {
  switch (msg.type) {
    case "client_setup":
    ...
    default: {
      ...
      throw new Error(`Unhandled message type: ${(_exhaustive as Draft09Message).type}`);
    }
  }
}

export function decodeMessage(bytes: Uint8Array): DecodeResult<Draft09Message> {
  try {
...
export interface Draft09Codec extends BaseCodec<Draft09Message> {
  readonly draft: "draft-ietf-moq-transport-09";
  ...
  createStreamDecoder(): TransformStream<Uint8Array, Draft09Message>;
}

export function createDraft09Codec(): Draft09Codec {
  return {
    draft: "draft-ietf-moq-transport-09",
    encodeMessage,
    decodeMessage,
