...
function encodeAuthorizationToken(at: AuthorizationToken, w: BufferWriter): void {
  ...
  if (aliasType === 0 || aliasType === 1 || aliasType === 2) {
    // DELETE, REGISTER, USE_ALIAS have token_alias
    tmpW.writeVarInt(at.token_alias ?? 0n)
  }
...
function decodeAuthorizationToken(r: BufferReader, length: number): AuthorizationToken {
...
function encodeSetupParams(params: Draft13SetupParams, w: BufferWriter): void {
  ...
  if (params.max_auth_token_cache_size !== undefined) {
    ...
    w.writeVarInt(params.max_auth_token_cache_size)
  }
  if (params.unknown) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (paramType % 2n === 0n) {
      ...
      if (paramType === SETUP_PARAM_PATH) {
        ...
        const token = decodeAuthorizationToken(r, length)
        noteAuthTokenValue(valueStart, length, token.token_value)
...
// Draft-13 subscribe_namespace (0x11) — byte-identical to the payload draft-12 spells subscribe_announces.
...
// Draft-13 Section 8.20: the TRACK_STATUS message format is identical to SUBSCRIBE.
...
/**
 * Overwrite every Authorization Token value in a draft-13 control frame.
 *
