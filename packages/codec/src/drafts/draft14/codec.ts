    ...
    if (paramType % 2n === 0n) {
      ...
      if (paramType === PARAM_PATH) {
        ...
        if (tokenReader.remaining > 0) {
          ...
          if (tokenReader.remaining > 0) {
            const value = new Uint8Array(tokenReader.readBytesView(tokenReader.remaining))
            tok.token_value = value
            noteAuthTokenValue(reader.offset - length, length, value)
          }
        }
...
/**
 * Overwrite every Authorization Token value in a draft-14 control frame.
 *
