...
import type { Draft18Message, Draft18MessageType } from './types.js'

function violation(
  code: ProtocolViolation<Draft18MessageType>['code'],
  message: string,
  currentPhase: SessionPhase,
  offendingMessage: Draft18MessageType,
): ProtocolViolation<Draft18MessageType> {
  return { code, message, currentPhase, offendingMessage }
}

export class Draft18SessionFSM {
  ...
  get legalOutgoing(): ReadonlySet<Draft18MessageType> {
    return getLegalOutgoing(this._phase, this._role)
  }

  get legalIncoming(): ReadonlySet<Draft18MessageType> {
    return getLegalIncoming(this._phase, this._role)
  }

  private checkDuplicateRequestId(
    requestId: bigint,
    msgType: Draft18MessageType,
  ): ProtocolViolation<Draft18MessageType> | null {
  ...
  validateOutgoing(message: Draft18Message): ValidationResult<Draft18MessageType> {
  ...
  receive(message: Draft18Message): TransitionResult<Draft18MessageType> {
    return this.applyTransition(message, 'inbound')
  }

  send(message: Draft18Message): TransitionResult<Draft18MessageType> {
    return this.applyTransition(message, 'outbound')
  }

  private applyTransition(
    message: Draft18Message,
    direction: 'inbound' | 'outbound',
  ): TransitionResult<Draft18MessageType> {
  ...
  private handleSetup(direction: 'inbound' | 'outbound'): TransitionResult<Draft18MessageType> {
    if (this._phase === 'idle') {
  ...
  private handleGoAway(
    ...
    const goaway = message as import('./types.js').Draft18GoAway
    sideEffects.push({
  ...
  private requireReady(msgType: Draft18MessageType): ProtocolViolation<Draft18MessageType> | null {
    if (this._phase !== 'ready' && this._phase !== 'draining') {
      return violation(
        this._phase === 'idle' || this._phase === 'setup'
          ? 'MESSAGE_BEFORE_SETUP'
  ...
  private handleSubscribe(
    ...
    const sub = message as import('./types.js').Draft18Subscribe
  ...
  private handleRequestUpdate(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const update = message as import('./types.js').Draft18RequestUpdate
  ...
  private handlePublish(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const pub = message as import('./types.js').Draft18Publish
  ...
  private handlePublishDone(
    ...
    if (err) return { ok: false, violation: err }
    // PUBLISH_DONE has no request_id; dequeue oldest active publish
  ...
  private handleFetch(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const fetch = message as import('./types.js').Draft18Fetch
  ...
  private handleFetchOk(sideEffects: SideEffect[]): TransitionResult<Draft18MessageType> {
  ...
  private handleRequestError(sideEffects: SideEffect[]): TransitionResult<Draft18MessageType> {
    ...
    // Could be for subscribe_namespace, subscribe_tracks, publish_namespace, track_status — allow through
    return { ok: true, phase: this._phase, sideEffects }
  }

  private handleRequestOk(sideEffects: SideEffect[]): TransitionResult<Draft18MessageType> {
    const err = this.requireReady('request_ok')
    if (err) return { ok: false, violation: err }
    // REQUEST_OK is the unified response (also serves as PUBLISH_OK alias) — try FIFO
    // dequeue from each queue. Falls through for namespace-scoped requests.
    const pubId = this.dequeuePending(this._pendingPublishes, this._publishes)
    if (pubId !== undefined) {
      const pub = this._publishes.get(pubId)!
  ...
  private handleReadyPhaseMessage(message: Draft18Message): TransitionResult<Draft18MessageType> {
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    return { ok: true, phase: this._phase, sideEffects: [] }
  }
