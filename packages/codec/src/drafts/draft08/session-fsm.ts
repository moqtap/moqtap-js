...
function violation(
  code: ProtocolViolation<Draft08MessageType>['code'],
  message: string,
  currentPhase: SessionPhase,
  offendingMessage: Draft08MessageType,
): ProtocolViolation<Draft08MessageType> {
  return { code, message, currentPhase, offendingMessage }
}

export class Draft08SessionFSM {
  ...
  get legalOutgoing(): ReadonlySet<Draft08MessageType> {
    return getLegalOutgoing(this._phase, this._role)
  }

  get legalIncoming(): ReadonlySet<Draft08MessageType> {
    return getLegalIncoming(this._phase, this._role)
  }

  private checkRole(
    message: Draft08Message,
    direction: 'inbound' | 'outbound',
  ): ProtocolViolation<Draft08MessageType> | null {
    const senderRole =
  ...
  validateOutgoing(message: Draft08Message): ValidationResult<Draft08MessageType> {
  ...
  receive(message: Draft08Message): TransitionResult<Draft08MessageType> {
    const roleViolation = this.checkRole(message, 'inbound')
    if (roleViolation) return { ok: false, violation: roleViolation }
    return this.applyTransition(message, 'inbound')
  }

  send(message: Draft08Message): TransitionResult<Draft08MessageType> {
    const roleViolation = this.checkRole(message, 'outbound')
    if (roleViolation) return { ok: false, violation: roleViolation }
  ...
  private applyTransition(
    message: Draft08Message,
    direction: 'inbound' | 'outbound',
  ): TransitionResult<Draft08MessageType> {
  ...
  private handleGoAway(
    ...
    const goaway = message as import('./types.js').Draft08GoAway
    sideEffects.push({
  ...
  private requireReady(msgType: Draft08MessageType): ProtocolViolation<Draft08MessageType> | null {
    if (this._phase !== 'ready' && this._phase !== 'draining') {
      return violation(
        this._phase === 'idle' || this._phase === 'setup'
          ? 'MESSAGE_BEFORE_SETUP'
  ...
  private handleSubscribe(
    ...
    const sub = message as import('./types.js').Draft08Subscribe
  ...
  private handleSubscribeOk(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const ok = message as import('./types.js').Draft08SubscribeOk
  ...
  private handleSubscribeError(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const subErr = message as import('./types.js').Draft08SubscribeError
  ...
  private handleSubscribeUpdate(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const update = message as import('./types.js').Draft08SubscribeUpdate
  ...
  private handleSubscribeDone(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const done = message as import('./types.js').Draft08SubscribeDone
  ...
  private handleUnsubscribe(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const unsub = message as import('./types.js').Draft08Unsubscribe
  ...
  private handleAnnounce(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const ann = message as import('./types.js').Draft08Announce
    const nsKey = ann.track_namespace.join('/')
    this._announces.set(nsKey, {
  ...
  private handleAnnounceOk(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const ok = message as import('./types.js').Draft08AnnounceOk
  ...
  private handleAnnounceError(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const annErr = message as import('./types.js').Draft08AnnounceError
  ...
  private handleUnannounce(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const unann = message as import('./types.js').Draft08Unannounce
  ...
  private handleAnnounceCancel(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const cancel = message as import('./types.js').Draft08AnnounceCancel
  ...
  private handleFetchOk(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const ok = message as import('./types.js').Draft08FetchOk
    const idErr = this.checkKnownSubscribeId(ok.subscribe_id, message.type)
    if (idErr) return { ok: false, violation: idErr }
  ...
  private handleFetchError(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const fetchErr = message as import('./types.js').Draft08FetchError
  ...
  private handleFetchCancel(
    ...
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    const cancel = message as import('./types.js').Draft08FetchCancel
  ...
  private handleReadyPhaseMessage(message: Draft08Message): TransitionResult<Draft08MessageType> {
    const err = this.requireReady(message.type)
    if (err) return { ok: false, violation: err }
    return { ok: true, phase: this._phase, sideEffects: [] }
  }
