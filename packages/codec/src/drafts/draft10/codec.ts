...
function encodeSetupParams(params: Draft10SetupParams, w: BufferWriter): void {
  let count = 0;
...
function decodeSetupParams(r: BufferReader): Draft10SetupParams {
  const count = Number(r.readVarInt());
  const result: Draft10SetupParams = {};
...
function encodeParams(params: Draft10Params, w: BufferWriter): void {
...
function decodeParams(r: BufferReader): Draft10Params {
  const count = Number(r.readVarInt());
  const result: Draft10Params = {};
  const unknown: UnknownParam[] = [];
...
function encodeClientSetupPayload(
  msg: Draft10Message & { type: "client_setup" },
  w: BufferWriter,
): void {
...
function encodeServerSetupPayload(
  msg: Draft10Message & { type: "server_setup" },
  w: BufferWriter,
): void {
...
function encodeSubscribePayload(
  msg: Draft10Message & { type: "subscribe" },
  w: BufferWriter,
): void {
...
function encodeSubscribeOkPayload(
  msg: Draft10Message & { type: "subscribe_ok" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
...
function encodeSubscribeErrorPayload(
  msg: Draft10Message & { type: "subscribe_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
...
function encodeSubscribeUpdatePayload(
  msg: Draft10Message & { type: "subscribe_update" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeVarInt(msg.start_group);
...
function encodeSubscribeDonePayload(
  msg: Draft10Message & { type: "subscribe_done" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
...
function encodeUnsubscribePayload(
  msg: Draft10Message & { type: "unsubscribe" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
}

function encodeAnnouncePayload(msg: Draft10Message & { type: "announce" }, w: BufferWriter): void {
  w.writeTuple(msg.track_namespace);
  encodeParams(msg.parameters, w);
}

function encodeAnnounceOkPayload(
  msg: Draft10Message & { type: "announce_ok" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
}
...
function encodeAnnounceErrorPayload(
  msg: Draft10Message & { type: "announce_error" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeVarInt(msg.error_code);
...
function encodeUnannouncePayload(
  msg: Draft10Message & { type: "unannounce" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
}
...
function encodeAnnounceCancelPayload(
  msg: Draft10Message & { type: "announce_cancel" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeVarInt(msg.error_code);
...
function encodeSubscribeAnnouncesPayload(
  msg: Draft10Message & { type: "subscribe_announces" },
  w: BufferWriter,
): void {
...
function encodeSubscribeAnnouncesOkPayload(
  msg: Draft10Message & { type: "subscribe_announces_ok" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace_prefix);
}
...
function encodeSubscribeAnnouncesErrorPayload(
  msg: Draft10Message & { type: "subscribe_announces_error" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace_prefix);
  w.writeVarInt(msg.error_code);
...
function encodeUnsubscribeAnnouncesPayload(
  msg: Draft10Message & { type: "unsubscribe_announces" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace_prefix);
}

function encodeFetchPayload(msg: Draft10Message & { type: "fetch" }, w: BufferWriter): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeUint8(msg.subscriber_priority);
  w.writeUint8(msg.group_order);
...
function encodeFetchOkPayload(msg: Draft10Message & { type: "fetch_ok" }, w: BufferWriter): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeUint8(msg.group_order);
...
function encodeFetchErrorPayload(
  msg: Draft10Message & { type: "fetch_error" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
  w.writeVarInt(msg.error_code);
...
function encodeFetchCancelPayload(
  msg: Draft10Message & { type: "fetch_cancel" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
}
...
function encodeTrackStatusRequestPayload(
  msg: Draft10Message & { type: "track_status_request" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeString(msg.track_name);
...
function encodeTrackStatusPayload(
  msg: Draft10Message & { type: "track_status" },
  w: BufferWriter,
): void {
  w.writeTuple(msg.track_namespace);
  w.writeString(msg.track_name);
...
function encodeGoAwayPayload(msg: Draft10Message & { type: "goaway" }, w: BufferWriter): void {
  w.writeString(msg.new_session_uri);
}

function encodeMaxSubscribeIdPayload(
  msg: Draft10Message & { type: "max_subscribe_id" },
  w: BufferWriter,
): void {
  w.writeVarInt(msg.subscribe_id);
}
...
function encodeSubscribesBlockedPayload(
  msg: Draft10Message & { type: "subscribes_blocked" },
  w: BufferWriter,
): void {
...
function decodeClientSetupPayload(r: BufferReader): Draft10Message {
...
function decodeServerSetupPayload(r: BufferReader): Draft10Message {
  const selected_version = r.readVarInt();
  const parameters = decodeSetupParams(r);
  return { type: "server_setup", selected_version, parameters };
}

function decodeSubscribePayload(r: BufferReader): Draft10Message {
  ...
  // Draft-09 removed LatestGroup (filter_type=1), valid values are 2-4
  if (ft < 2 || ft > 4) {
  ...
  return msg as unknown as Draft10Message;
}

function decodeSubscribeOkPayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
...
function decodeSubscribeErrorPayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
...
function decodeSubscribeUpdatePayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
...
function decodeSubscribeDonePayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
...
function decodeUnsubscribePayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
  return { type: "unsubscribe", subscribe_id };
}

function decodeAnnouncePayload(r: BufferReader): Draft10Message {
  const track_namespace = r.readTuple();
  const parameters = decodeParams(r);
  return { type: "announce", track_namespace, parameters };
}

function decodeAnnounceOkPayload(r: BufferReader): Draft10Message {
  const track_namespace = r.readTuple();
  return { type: "announce_ok", track_namespace };
}

function decodeAnnounceErrorPayload(r: BufferReader): Draft10Message {
  const track_namespace = r.readTuple();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeUnannouncePayload(r: BufferReader): Draft10Message {
  const track_namespace = r.readTuple();
  return { type: "unannounce", track_namespace };
}

function decodeAnnounceCancelPayload(r: BufferReader): Draft10Message {
  const track_namespace = r.readTuple();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeSubscribeAnnouncesPayload(r: BufferReader): Draft10Message {
  const track_namespace_prefix = r.readTuple();
  const parameters = decodeParams(r);
  return { type: "subscribe_announces", track_namespace_prefix, parameters };
}

function decodeSubscribeAnnouncesOkPayload(r: BufferReader): Draft10Message {
  const track_namespace_prefix = r.readTuple();
  return { type: "subscribe_announces_ok", track_namespace_prefix };
}

function decodeSubscribeAnnouncesErrorPayload(r: BufferReader): Draft10Message {
  const track_namespace_prefix = r.readTuple();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeUnsubscribeAnnouncesPayload(r: BufferReader): Draft10Message {
  const track_namespace_prefix = r.readTuple();
  return { type: "unsubscribe_announces", track_namespace_prefix };
}

function decodeFetchPayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
  const subscriber_priority = r.readUint8();
  const group_order = r.readUint8();
...
function decodeFetchOkPayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
  const group_order = r.readUint8();
...
function decodeFetchErrorPayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
  const error_code = r.readVarInt();
  const reason_phrase = r.readString();
...
function decodeFetchCancelPayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
  return { type: "fetch_cancel", subscribe_id };
}

function decodeTrackStatusRequestPayload(r: BufferReader): Draft10Message {
  const track_namespace = r.readTuple();
  const track_name = r.readString();
  return { type: "track_status_request", track_namespace, track_name };
}

function decodeTrackStatusPayload(r: BufferReader): Draft10Message {
  const track_namespace = r.readTuple();
  const track_name = r.readString();
  const status_code = r.readVarInt();
...
function decodeGoAwayPayload(r: BufferReader): Draft10Message {
  const new_session_uri = r.readString();
  return { type: "goaway", new_session_uri };
}

function decodeMaxSubscribeIdPayload(r: BufferReader): Draft10Message {
  const subscribe_id = r.readVarInt();
  return { type: "max_subscribe_id", subscribe_id };
}

function decodeSubscribesBlockedPayload(r: BufferReader): Draft10Message {
...
const payloadDecoders: ReadonlyMap<bigint, (r: BufferReader) => Draft10Message> = new Map([
...
export function encodeMessage(message: Draft10Message): Uint8Array {
...
function encodePayload(msg: Draft10Message, w: BufferWriter): void {
  switch (msg.type) {
    case "client_setup":
    ...
    default: {
      ...
      throw new Error(`Unhandled message type: ${(_exhaustive as Draft10Message).type}`);
    }
  }
}

export function decodeMessage(bytes: Uint8Array): DecodeResult<Draft10Message> {
  try {
...
export function encodeDatagram(dg: DatagramObject): Uint8Array {
  ...
  // extension_headers_length (0 = no extensions)
  w.writeVarInt(0n);
  // remaining bytes = payload (no payload_length field in draft-10)
...
export function createStreamDecoder(): TransformStream<Uint8Array, Draft10Message> {
  let buffer = new Uint8Array(0);
  return new TransformStream<Uint8Array, Draft10Message>({
...
export interface Draft10Codec extends BaseCodec<Draft10Message> {
  readonly draft: "draft-ietf-moq-transport-10";
  ...
  createStreamDecoder(): TransformStream<Uint8Array, Draft10Message>;
}

export function createDraft10Codec(): Draft10Codec {
  return {
    draft: "draft-ietf-moq-transport-10",
    encodeMessage,
    decodeMessage,
