...
import type { DatagramObject, FetchStream, SubgroupStream } from '../../drafts/draft15/types.js'
import { bytesToHex, hexToBytes, loadVectorDir } from '../helpers.js'

const codec = createDraft15Codec()

const vectorEntries = loadVectorDir('transport/draft15/codec/data-streams')

for (const { file, data: vectorFile } of vectorEntries) {
  const messageType = vectorFile.message_type

  describe(`draft-15 data stream: ${messageType} (${file})`, () => {
...
function getStreamType(
  ...
  // Infer from file name / message type
  if (messageType === 'object') return 'subgroup' // subgroup.json uses message_type "object"
...
function assertDataStreamMatch(
  ...
  if (expected.subscribe_request_id !== undefined) {
    expect(String(a.requestId)).toBe(String(expected.subscribe_request_id))
  }
  ...
  // Check datagram-specific fields
  ...
  // Check objects array for subgroup/fetch streams
