...
const codec = createDraft08Codec()

const vectorEntries = loadVectorDir('transport/draft08/codec/data-streams')

for (const { file, data: vectorFile } of vectorEntries) {
  const messageType = vectorFile.message_type

  describe(`draft-08 data stream: ${messageType} (${file})`, () => {
...
function assertDataStreamMatch(
  ...
  if (expected.extension_count !== undefined) {
    expect(String(a.extensionCount)).toBe(String(expected.extension_count))
  }
  ...
  if (expected.objects !== undefined) {
    ...
    for (let i = 0; i < expectedObjects.length; i++) {
      ...
      if (eo.extension_count !== undefined) {
        expect(String(ao.extensionCount)).toBe(String(eo.extension_count))
      }
