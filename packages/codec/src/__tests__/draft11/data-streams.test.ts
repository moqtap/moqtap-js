...
import type { DatagramObject, FetchStream, SubgroupStream } from '../../drafts/draft11/types.js'
import { bytesToHex, hexToBytes, loadVectorDir } from '../helpers.js'

const codec = createDraft11Codec()

const vectorEntries = loadVectorDir('transport/draft11/codec/data-streams')

for (const { file, data: vectorFile } of vectorEntries) {
  const messageType = vectorFile.message_type

  describe(`draft-11 data stream: ${messageType} (${file})`, () => {
