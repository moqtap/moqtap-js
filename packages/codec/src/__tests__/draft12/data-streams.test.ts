...
import type { DatagramObject, FetchStream, SubgroupStream } from '../../drafts/draft12/types.js'
import { bytesToHex, hexToBytes, loadVectorDir } from '../helpers.js'

const codec = createDraft12Codec()

const vectorEntries = loadVectorDir('transport/draft12/codec/data-streams')

for (const { file, data: vectorFile } of vectorEntries) {
  const messageType = vectorFile.message_type

  describe(`draft-12 data stream: ${messageType} (${file})`, () => {
...
function assertDataStreamMatch(
  ...
  if (expected.payload_length !== undefined) {
