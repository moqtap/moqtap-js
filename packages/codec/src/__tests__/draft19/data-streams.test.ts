...
import type { DatagramObject, FetchStream, SubgroupStream } from '../../drafts/draft19/types.js'
import { bytesToHex, hexToBytes, loadVectorDir } from '../helpers.js'

const codec = createDraft19Codec()

const vectorEntries = loadVectorDir('transport/draft19/codec/data-streams')

for (const { file, data: vectorFile } of vectorEntries) {
  const messageType = vectorFile.message_type

  describe(`draft-19 data stream: ${messageType} (${file})`, () => {
