...
import type { SubgroupStream, DatagramObject, FetchStream } from '../drafts/draft14/types.js';
...
function getStreamType(messageType: string, vector: { decoded?: Record<string, unknown> }): 'subgroup' | 'datagram' | 'fetch' {
...
function assertDataStreamMatch(
  ...
  if (expected['subscribe_request_id'] !== undefined) {
    expect(String(a['subscribeRequestId'])).toBe(String(expected['subscribe_request_id']));
  }
  ...
  if (expected['payload_hex'] !== undefined) {
    ...
    expect(String(a['payloadLength'])).toBe(String(expected['payload_length']));
  }
