    ...
    for (const vector of vectorFile.vectors) {
      describe(`[${vector.id}] ${vector.description}`, () => {
        ...
        if (vector.error) {
          ...
          const streamType = mapStreamType(decoded.stream_type as string)
...
/** Map vector stream_type names to codec dispatch types */
function mapStreamType(vectorStreamType: string): 'subgroup' | 'datagram' | 'fetch' {
  switch (vectorStreamType) {
    case 'subgroup_header':
    case 'subgroup':
      return 'subgroup'
    case 'object_datagram':
    case 'object_datagram_status':
    case 'datagram':
      return 'datagram'
    case 'fetch_header':
    case 'fetch':
      return 'fetch'
    default:
      return vectorStreamType as 'subgroup' | 'datagram' | 'fetch'
  }
}

function getStreamType(vector: {
  decoded?: Record<string, unknown>
}): 'subgroup' | 'datagram' | 'fetch' {
  if (vector.decoded?.stream_type) {
    return mapStreamType(vector.decoded.stream_type as string)
  }
  return 'subgroup'
}
...
function assertSubgroupMatch(
  ...
  // Stream type ID maps to headerType
  if (expected.stream_type_id !== undefined) {
    expect(String(actual.headerType)).toBe(String(expected.stream_type_id))
  }
  ...
  if (expected.objects !== undefined) {
    ...
    for (let i = 0; i < expectedObjects.length; i++) {
      ...
      if (eo.extension_headers_length !== undefined) {
        expect(String((ao.extensionData as Uint8Array).length)).toBe(
          String(eo.extension_headers_length),
        )
      }
      if (eo.extension_headers_hex !== undefined) {
        expect(bytesToHex(ao.extensionData as Uint8Array)).toBe(eo.extension_headers_hex)
      }
...
function assertDatagramMatch(
  ...
  if (expected.stream_type_id !== undefined) {
    expect(String(actual.datagramType)).toBe(String(expected.stream_type_id))
  }
  if (expected.track_alias !== undefined) {
    expect(String(actual.trackAlias)).toBe(String(expected.track_alias))
  }
