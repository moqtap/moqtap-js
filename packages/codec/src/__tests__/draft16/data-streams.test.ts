...
import type { DatagramObject, FetchStream, SubgroupStream } from '../../drafts/draft16/types.js'
import { bytesToHex, hexToBytes, loadVectorDir } from '../helpers.js'

const codec = createDraft16Codec()

const vectorEntries = loadVectorDir('transport/draft16/codec/data-streams')

for (const { file, data: vectorFile } of vectorEntries) {
  const messageType = vectorFile.message_type

  describe(`draft-16 data stream: ${messageType} (${file})`, () => {
