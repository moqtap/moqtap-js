...
import { createDraft21Codec, UNKNOWN_STREAM_COUNT } from '../../drafts/draft21/codec.js'
import type { Draft21Message } from '../../drafts/draft21/types.js'
import {
  bytesToHex,
  hexToBytes,
  loadVectorDir,
...
const vectorEntries = loadVectorDir('transport/draft21/codec/messages')
 ...
 * published contents of transport/draft21/codec/messages, and both have to be
 * updated deliberately.
 */
const EXPECTED_FILES = 21
const EXPECTED_VECTORS = 235

describe('draft-21 message vector corpus', () => {
...
for (const { file, data: vectorFile } of vectorEntries) {
  ...
  describe(`draft-21 ${messageType} (${file})`, () => {
 ...
 *
 * Every key the vector states is checked, at every depth — draft-21 nests
...
function assertMatches(actual: unknown, expected: unknown, path: string): void {
  ...
  // paths worth checking: draft-21's FILL_PARAMETERS holds one.
...
describe('draft-21 encoder specifics', () => {
  function encodeHex(message: Draft21Message): string {
  ...
  it('rejects a joining-style FETCH: draft-21 FETCH has no Fetch Type field', () => {
