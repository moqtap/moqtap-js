/**
 * The session rules the vector corpus cannot reach: fill fetch streams carry
 * no control message of their own, and PUBLISH_STATE_NOTIFY's rules are about
 * which stream it arrived on, which a single-message vector has no way to
 * express. They live in the session state machine rather than in the byte
 * layer, so this is where they are tested.
 */

import { describe, expect, it } from 'vitest'
import { createDraft21SessionState } from '../../drafts/draft21/session.js'
import type { Draft21Message, Draft21Params } from '../../drafts/draft21/types.js'

function ready(role: 'client' | 'server' = 'client') {
  const fsm = createDraft21SessionState(role)
...
function subscribe(requestId: bigint, parameters: Draft21Params): Draft21Message {
  return {
    type: 'subscribe',
...
describe('draft-21 fill fetch streams (Section 5.1.3)', () => {
...
describe('draft-21 PUBLISH_STATE_NOTIFY (Section 10.10)', () => {
  const notify: Draft21Message = {
