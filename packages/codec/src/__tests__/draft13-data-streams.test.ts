...
import type { DatagramObject, FetchStream, SubgroupStream } from "../drafts/draft13/types.js";
import { bytesToHex, hexToBytes, loadVectorDir } from "./helpers.js";

const codec = createDraft13Codec();

const vectorEntries = loadVectorDir("transport/draft13/codec/data-streams");

for (const { file, data: vectorFile } of vectorEntries) {
  const messageType = vectorFile.message_type;

  describe(`draft-13 data stream: ${messageType} (${file})`, () => {
