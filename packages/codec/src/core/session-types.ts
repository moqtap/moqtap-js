...
export type SessionStateOptionsRole = 'client' | 'server'

export interface SessionStateOptions<T extends Draft> {
  codec: { draft: T }
  role: SessionStateOptionsRole
}

export interface SessionState<M, T extends string> {
...
export interface ProtocolViolation<T extends string> {
